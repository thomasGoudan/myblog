...
public class EsBlogServiceImpl implements EsBlogService {
    ...
    public List<TagVO> listTop30Tags() {
        ...
        SearchQuery searchQuery = new NativeSearchQueryBuilder()
                ...
                .withIndices("esblogss").withTypes("esblogs")
...
//        StringTerms modelTerms =  (StringTerms)aggregations.asMap().get("tags");
        Aggregation aggregation = aggregations.asMap().get("tags");
        Terms teamSum= (Terms)aggregation;

        Iterator<? extends Terms.Bucket> iterator = teamSum.getBuckets().iterator();
        while (iterator.hasNext()) {
            Terms.Bucket next = iterator.next();

            list.add(new TagVO(next.getKey().toString(),
                    next.getDocCount()));
        }
        return list;
    }
